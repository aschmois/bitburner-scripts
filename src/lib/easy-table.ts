...
export abstract class Printer {
  public static currency(g: Global, opts?: Opts | undefined) {
    return (val: number, width: number) => {
      const str = applyOpts(g.n(val, '$0.00a'), opts)
      return width ? EasyTable.padLeft(str, width) : str
    }
  }
  public static number(g: Global, opts?: Opts | undefined) {
    return (val: number, width: number) => {
      const str = applyOpts(g.n(val, '0,0'), opts)
      return width ? EasyTable.padLeft(str, width) : str
    }
  }
  public static nNumber(g: Global, opts?: Opts | undefined) {
    return (val: number, width: number) => {
      const str = applyOpts(g.n(val, '0.00a'), opts)
      return width ? EasyTable.padLeft(str, width) : str
    }
  }
  public static percent(g: Global, opts?: Opts | undefined) {
    return (val: number, width: number) => {
...
function applyOpts(_str: string, opts?: Opts | undefined) {
  const pre = opts?.pre || ''
